...
These tests drive the host Docker daemon directly and also need host FFmpeg,
so they run from the host, not inside the `vms` container (that image
deliberately ships neither a Docker client nor FFmpeg):
