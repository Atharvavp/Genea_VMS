...
```bash
cd services/vms               # every command below runs from this directory
...
(cd ../rtsp-simulator && docker compose up -d --build)   # http://localhost:8080
...
Integration and browser tests are opt-in and run **from the host**: they drive
the Docker daemon and host FFmpeg, and the `vms` image deliberately ships
neither. Create the host environment once:
...
```bash
.venv/bin/python -m pytest -q                            # unit / API / static UI
